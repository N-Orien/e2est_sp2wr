...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument('--valid-mixed-json', type=str, default=None,
                        help='Filename of validation label data (json)')
...
def main(cmd_args):
    ...
    if args.num_spkrs == 1:
        if args.backend == "chainer":
            from espnet.asr.chainer_backend.asr_mixed_valid import train
            train(args)
        elif args.backend == "pytorch":
            from espnet.asr.pytorch_backend.asr_mixed_valid import train
            train(args)
        else:
