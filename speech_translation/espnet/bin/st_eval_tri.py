...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument('--dict-wrsrc', required=required,
                        help='Written source dictionary')
    ...
    parser.add_argument('--asr-weight', default=0.0, type=float,
                        help='Multitask learning coefficient for ASR task, weight: \
                            ...
                            (1-alpha)*att_loss) + conv_weight*conv_loss + (1-asr_weight-conv_weight)*st_loss')
    parser.add_argument('--conv-weight', default=0.0, type=float,
                        help='Multitask learning coefficient for conversion task, weight: \
                            asr_weight*(alpha*ctc_loss + \
                            (1-alpha)*att_loss) + conv_weight*conv_loss + (1-asr_weight-conv_weight)*st_loss')
    ...
    parser.add_argument('--dec-asr-init', default=None, type=str, nargs='?',
                        help='Pre-trained model to initialize ASR decoder.')
    parser.add_argument('--dec-conv-init', default=None, type=str, nargs='?',
                        help='Pre-trained model to initialize conversion decoder.')
    ...
    parser.add_argument('--use-lid', default=True, type=strtobool,
    ...
    parser.add_argument('--cross-st2asr', type=strtobool, default=False,
                        help='Enable cross attention for the ASR decoder. \
                            ASR decoder attends to ST decoder.')
    parser.add_argument('--cross-asr2st', type=strtobool, default=False,
                        help='Enable cross attention for the ST decoder. \
                            ST decoder attends to ASR decoder.')
    parser.add_argument('--cross-st2conv', type=strtobool, default=False,
                        help='Enable cross attention for the conversion decoder. \
                            conversion decoder attends to ST decoder.')
    parser.add_argument('--cross-conv2st', type=strtobool, default=False,
                        help='Enable cross attention for the ST decoder. \
                            ST decoder attends to conversion decoder.')
    parser.add_argument('--cross-conv2asr', type=strtobool, default=False,
                        help='Enable cross attention for the ASR decoder. \
                                ASR decoder attends to conversion decoder.')
    parser.add_argument('--cross-asr2conv', type=strtobool, default=False,
                        help='Enable cross attention for the conversion decoder. \
                            conversion decoder attends to ASR decoder.')
    ...
    parser.add_argument('--wait-k-asr', default=0, type=int,
                        help='ASR decoder is k steps ahead of conversion decoder.')
    parser.add_argument('--wait-k-conv', default=0, type=int,
                        help='conversion decoder is k steps ahead of ST decoder.')
...
def main(cmd_args):
    ...
    if args.dict_src is not None and args.dict_tgt is not None and args.dict_wrsrc is not None:
        if args.dict_src == args.dict_wrsrc and args.dict_wrsrc == args.dict_tgt:
            logging.info('*** Use JOINT dictionary for source, written source and target ***')
            ...
            args.char_list_wrsrc = char_list
        ...
        args.char_list_wrsrc = None
        args.char_list_tgt = None

    args.enc_init += str(args.index)
    args.dec_init += str(args.index)
    args.dec_asr_init += str(args.index)
    args.dec_conv_init += str(args.index)

    # train
    logging.info('backend = ' + args.backend)

    if args.backend == "pytorch":
        from espnet.st.pytorch_backend.st_tri import eval
        eval(args)
    else:
