...
def train(args):
    ...
    with open(args.valid_json, 'rb') as f:
        valid_json = json.load(f)['utts']
    with open(args.valid_mixed_json, 'rb') as f:
        valid_mixed_json = json.load(f)['utts']
    ...
    valid = make_batchset(valid_json, args.batch_size,
                          ...
                          batch_frames_inout=args.batch_frames_inout,
                          iaxis=0, oaxis=0)
    valid_mixed = make_batchset(valid_mixed_json, args.batch_size,
                          args.maxlen_in, args.maxlen_out, args.minibatches,
                          min_batch_size=args.ngpu if args.ngpu > 1 else 1,
                          count=args.batch_count,
                          batch_bins=args.batch_bins,
    ...
    valid_mixed_iter = {'main': ChainerDataLoader(
        dataset=TransformDataset(valid_mixed, lambda data: converter([load_cv(data)])),
        batch_size=1, shuffle=False, collate_fn=lambda x: x[0],
        num_workers=args.n_iter_processes)}
    ...
    if args.save_interval_iters > 0:
        ...
        trainer.extend(CustomEvaluator(model, valid_mixed_iter, reporter, device, args.ngpu),
                       trigger=(args.save_interval_iters, 'iteration'))
    else:
        trainer.extend(CustomEvaluator(model, valid_iter, reporter, device, args.ngpu))
