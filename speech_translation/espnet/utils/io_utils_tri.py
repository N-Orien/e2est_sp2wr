...
class LoadInputsAndTargets(object):
    ...
    def __init__(self, mode='asr',
        ...
        self.langs_dict_wrsrc = langs_dict_wrsrc
                    ...
                    if 'tokenid' in inp:
                        if self.mode != 'mt':
                            ...
                            x = np.fromiter(map(int, inp['tokenid'].split()),
                                            dtype=np.int64)
                            # Added lang_id to x
                            if self.langs_dict_tgt is not None and self.langs_dict_src is not None and self.langs_dict_wrsrc is not None:
                                if inp['name'] == 'target1':
                                    x = (self.langs_dict_tgt['<2'+info['lang']+'>'], x)
                                elif inp['name'] == 'target2':
                                    x = (self.langs_dict_src[f'<2{self.src_lang}>'], x)
                                elif inp['name'] == 'target3':
                                    x = (self.langs_dict_wrsrc[f'<2{self.src_lang}>'], x)
                        else:
                            if inp['name'] == 'target1':
                                x = np.fromiter(map(int, inp['tokenid'].split()),
                                                dtype=np.int64)
