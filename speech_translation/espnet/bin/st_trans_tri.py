...
def get_parser():
    ...
    parser.add_argument('--beam-size-asr', type=int, default=1,
                        help='Beam size')
    parser.add_argument('--beam-size-conv', type=int, default=1,
                        help='Beam size')
    parser.add_argument('--penalty-asr', type=float, default=0.0,
                        help='Incertion penalty')
    parser.add_argument('--penalty-conv', type=float, default=0.0,
                        help='Incertion penalty')
    parser.add_argument('--maxlenratio-asr', type=float, default=0.0,
                        help="""Input length ratio to obtain max output length.
                        If maxlenratio=0.0 (default), it uses a end-detect function
                        to automatically find maximum hypothesis lengths""")
    parser.add_argument('--maxlenratio-conv', type=float, default=0.0,
                        help="""Input length ratio to obtain max output length.
                        If maxlenratio=0.0 (default), it uses a end-detect function
                        to automatically find maximum hypothesis lengths""")
    parser.add_argument('--minlenratio-asr', type=float, default=0.0,
                        help='Input length ratio to obtain min output length')
    parser.add_argument('--minlenratio-conv', type=float, default=0.0,
                        help='Input length ratio to obtain min output length')
    ...
    parser.add_argument('--recog-conv-and-trans', default=False, type=strtobool)
    parser.add_argument('--use-rev-triu-width', default=0, type=int)
    parser.add_argument('--use-diag', default=False, type=strtobool)
    parser.add_argument('--beam-search-type', type=str, choices=['sum', 'st', 'separate', 'sum-mono', 'half-joint'], default='st',
    ...
    parser.add_argument('--decode-asr-weight', type=float, default=1.0,
                        help='Input length ratio to obtain min output length')
    parser.add_argument('--decode-conv-weight', type=float, default=1.0,
                        help='Input length ratio to obtain min output length')
    ...
    parser.add_argument('--ratio-diverse-asr', type=float, default=0.0,
                        help='Input length ratio to obtain min output length')
    parser.add_argument('--ratio-diverse-conv', type=float, default=0.0,
                        help='Input length ratio to obtain min output length')
    parser.add_argument('--wait-k-asr', type=int, default=0,
                        help='ASR waits conversion for k steps.')
    parser.add_argument('--wait-k-conv', type=int, default=0,
                        help='conversion waits ST for k steps.')
    # rnnlm related
...
def main(args):
    ...
    if args.backend == "pytorch":
        ...
        from espnet.st.pytorch_backend.st_tri import trans
