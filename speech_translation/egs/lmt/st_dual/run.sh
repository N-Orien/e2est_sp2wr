...
if [[ ${stage} -le 1 ]] && [[ ${stop_stage} -ge 1 ]]; then
	...
	fbankdir="fbank"
        ...
        for x in train_sp dev; do
            ...
            comm -12 data/${x}.en.tmp/reclist1 data/${x}.en.tmp/reclist2 > data/${x}.sp.tmp/reclist
...
if [[ ${stage} -le 2 ]] && [[ ${stop_stage} -ge 2 ]]; then
    ...
    # Create separate dictionaries: 1 for source transcription, 1 joint for all target langs
        ...
        nlsyms=data/lang_1spm/use_${dprefix}/${train_set}_non_lang_syms_${tgt_case}
        nlsyms_tmp=data/lang_1spm/use_${dprefix}/${train_set}_non_lang_syms_tmp_${tgt_case} 
        dname=${train_set}_${bpemode}_src${nbpe_src}${src_case}_tgt${nbpe}${tgt_case}
        bpemodel=data/lang_1spm/use_${dprefix}/${dname}
        ...
        grep sp1.0 data/train_sp.en/text.${tgt_case} | cut -f 2- -d' ' | grep -o -P '&[^;]*;' >> ${nlsyms_tmp_tgt}
	...
	if [[ ${use_lid} == "true" ]]; then
        ...
        echo "| offset= ${offset}"
        input_path=data/lang_1spm/use_${dprefix}/input_${dprefix}_tgt_${tgt_case}.txt
        if [ -f ${input_path} ]; then
            echo "remove existing input text file"
            rm ${input_path}
        fi
	...
	echo "make json files"
            ...
            jname=data_${dprefix}_${bpemode}_src${nbpe_src}${src_case}_tgt${nbpe}${tgt_case}.json
        ...
        grep sp1.0 data/train_sp.sp/text.${src_case} | cut -f 2- -d' ' | grep -o -P '&[^;]*;' >> ${nlsyms_tmp_src}
        ...
        echo "| offset= ${offset}"
        input_path=data/lang_1spm/use_${dprefix}/input_${dprefix}_src_${src_case}.txt
        if [ -f ${input_path} ]; then
            echo "remove existing input text file"
            rm ${input_path}
        fi
...
datadir=${datadir}/use_${dprefix}/src${nbpe_src}_tgt${nbpe}
