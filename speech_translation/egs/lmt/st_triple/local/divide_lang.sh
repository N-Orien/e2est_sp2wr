...
for f in spk2utt utt2spk segments wav.scp feats.scp utt2num_frames; do
    if [ -f data/${set}/${f} ]; then
        sort data/${set}/${f} > data/${set}.wr/${f}
    fi
done
sort data/${set}/text.tc.wr > data/${set}.wr/text  # dummy
sort data/${set}/text.tc.wr > data/${set}.wr/text.tc
sort data/${set}/text.lc.wr > data/${set}.wr/text.lc
sort data/${set}/text.lc.rm.wr > data/${set}.wr/text.lc.rm
utils/fix_data_dir.sh --utt_extra_files "text.tc text.lc text.lc.rm" data/${set}.wr
if [ -f data/${set}.wr/feats.scp ]; then
    utils/validate_data_dir.sh data/${set}.wr || exit 1;
else
    utils/validate_data_dir.sh --no-feats --no-wav data/${set}.wr || exit 1;
fi
