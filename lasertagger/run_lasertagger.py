...
def main(_):
  ...
  if FLAGS.do_eval:
    ...
    for ckpt in ckpt_state.all_model_checkpoint_paths:
      ...
      if best_acc < result["sentence_level_acc"]:
        best_acc = result["sentence_level_acc"]
        with open(Path(FLAGS.output_dir) / "best_acc.txt", "w") as f:
          print(ckpt, file=f)
  ...
  if FLAGS.do_export:
    ...
    ckpt = ""
    with open(Path(FLAGS.output_dir) / "best_acc.txt") as f:
      ckpt = f.readline().strip()
