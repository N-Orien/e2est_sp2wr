...
for set in train dev test; do
    ...
    awk '{
        duration=$3; offset=$5; spkid=$7; wavfile=$9
        gsub(",","",duration);
        gsub(",","",offset);
        gsub(",","",spkid);
	gsub("}","",wavfile);
        ...
        printf("lmt_%s@%s@%07.0f@%07.0f\n", spkid, wavfile, int(1000*startt+0.5), int(1000*endt+0.5));
    }' ${dst}/.yaml1 > ${dst}/.yaml2
    ...
    awk '{
        ...
        fileid=s[1] "@" s[2]; startf=s[3]; endf=s[4];
        printf("%s %s %.2f %.2f\n", segment, fileid, startf/1000, endf/1000);
    }' < ${dst}/text.tc.en | uniq | sort > ${dst}/segments

    awk '{
        segment=$1; split(segment,s,"[@]");
        fileid=s[1] "@" s[2]; wavfile=s[2]
        printf("%s cat '${wav_dir}'/%s |\n", fileid, wavfile);
    ...
    cut -d ' ' -f 1 ${dst}/text.tc.en | sort | uniq -c | sort -n -k1 -r | grep -v '1 lmt' \
        | sed 's/^[ \t]*//' > ${dst}/duplicate_lines
    cut -d ' ' -f 1 ${dst}/text.tc.en | sort | uniq -c | sort -n -k1 -r | grep '1 lmt' \
    ...
    utils/fix_data_dir.sh --utt_extra_files \
        "text.tc.en text.lc.en text.lc.rm.en text.tc.sp text.lc.sp text.lc.rm.sp" \
        data/${set}
